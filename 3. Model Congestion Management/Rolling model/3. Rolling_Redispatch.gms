$setglobal    on_SLP "*"      if "*" SLP input is readed, else ""
$setglobal    on_ARIMA ""      if "*" ARIMA input is readed, else ""
$setglobal    on_Normal   ""      if "*" Normal input is readed, else ""
...
RD_eq1..                            RD_OF  =e=
                                    ...
                                    + sum((g,t)$rh(t), RD_Pg_up(g,t)  * GenD(g,'var'))
                                    + sum((g,t)$rh(t), (RD_Pg_down(g,t) ) * (MP_DH(t) - GenD(g,'var')))
                                    
                                    + sum((g,t)$rh(t), RD_Qg_pos(g,t) * MP_DH(t))
                                    + sum((g,t)$rh(t), RD_Qg_neg(g,t) * MP_DH(t))
                                    + sum((p,t)$rh(t), RD_Qpv_pos(p,t) * MP_DH(t))
                                    + sum((p,t)$rh(t), RD_Qpv_neg(p,t) * MP_DH(t))
                                    
                                    + sum((b,t)$rh(t), RD_dem_resp_down(b,t) * (MP_DH(t)))
                                    + sum((b,t)$rh(t), RD_dem_resp_up(b,t) * MP_DH(t))
                                    + sum((b,t)$rh(t), RD_curtail(b,t) * MP_DH(t))
                                                              
                                    + sum((l,t)$rh(t),  violation(l,t) * 100000000)
;                                 
...
*                                    + RD_dem_resp_up(b,t)
                                    ...
                                    + sum(b, ((-1) * PTDF_UKQK(b,bb,t)*1000) * (sum(g$Map_G(b,g), RD_Qg_pos(g,t) - RD_Qg_neg(g,t))
                                    +  sum(p$Map_PV(b,p), RD_Qpv_pos(p,t) - RD_Qpv_neg(p,t))))
...
RD_eq5(b,t)$rh(t)..                       adjust_v(b,t)*1000  =l= ukn(b,t)*1.05
;

RD_eq6(b,t)$rh(t)..                       adjust_v(b,t)*1000  =g= ukn(b,t)*0.95
;

* active Redispatch positive active demand response constraint

RD_eq7(b,t)$rh(t)..                         RD_dem_resp_down(b,t) =l= Load_bus_p(b,t) * 0.5
;

* active Redispatch negative demand response constraint

RD_eq8(b,t)$rh(t)..                          RD_dem_resp_up(b,t) =l=  Load_bus_p(b,t) * 0.1
;
* active Redispatch Ramping up constraint 

RD_eq9(g,t)$rh(t)..                          RD_Pg_up(g,t)   =l= GenD(g,'pmax') - D_genP(g,t)
;
   
* active Redispatch Ramping down constraint 2 

RD_eq10(g,t)$rh(t)..                        RD_Pg_down(g,t) =l= D_genP(g,t)
;

* active Redispatch Curtailment

RD_eq11(b,t)$rh(t)..                        RD_curtail(b,t) =l= pv_infeed(b,t)
;
...
Report_RD_costs(iitt) =   sum((g,t), RD_Pg_up.l(g,t)  * GenD(g,'var'))
                    + sum((g,t), (RD_Pg_down.l(g,t) ) * (MP_DH(t) - GenD(g,'var')/2))
                                    
                    + sum((g,t), RD_Qg_pos.l(g,t) * MP_DH(t))
                    + sum((g,t), RD_Qg_neg.l(g,t) * MP_DH(t))
                    + sum((p,t), RD_Qpv_pos.l(p,t) * MP_DH(t))
                    + sum((p,t), RD_Qpv_neg.l(p,t) * MP_DH(t))
                                    
                    + sum((b,t), RD_dem_resp_down.l(b,t) * (MP_DH(t)))
                    + sum((b,t), RD_dem_resp_up.l(b,t) * MP_DH(t))
                    + sum((b,t), RD_curtail.l(b,t) * MP_DH(t));
    
                
Report_RD_bus_costs(iitt,b) =  sum((g,t)$Map_G(b,g), RD_Pg_up.l(g,t)  * GenD(g,'var'))
                    + sum((g,t)$Map_G(b,g), (RD_Pg_down.l(g,t) ) * (MP_DH(t) - GenD(g,'var')/2))
                                    
                    + sum((g,t)$Map_G(b,g), RD_Qg_pos.l(g,t) * MP_DH(t))
                    + sum((g,t)$Map_G(b,g), RD_Qg_neg.l(g,t) * MP_DH(t))
                    + sum((p,t)$Map_PV(b,p), RD_Qpv_pos.l(p,t) * MP_DH(t))
                    + sum((p,t)$Map_PV(b,p), RD_Qpv_neg.l(p,t) * MP_DH(t))
                                    
                    + sum((t), RD_dem_resp_down.l(b,t) * (MP_DH(t)))
                    + sum((t), RD_dem_resp_up.l(b,t) * MP_DH(t))
                    + sum((t), RD_curtail.l(b,t) * MP_DH(t));
...
;
Report_Total_RD_bus_costs(b) = sum(it,Report_RD_bus_costs(it,b))
;
execute_unload "Rolling_Redispatch_SLP.gdx"
;
